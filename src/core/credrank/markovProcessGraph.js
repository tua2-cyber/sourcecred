...
export class MarkovProcessGraph {
  ...
  _personalAttributionsMap: PersonalAttributionsMap;
  ...
  constructor(
    ...
    radiationTransitionProbabilities: $ReadOnlyArray<number>,
    personalAttributionsMap: PersonalAttributionsMap
  ) {
    this._nodes = nodes;
    this._edges = edges;
    ...
    this._personalAttributionsMap = personalAttributionsMap;
  }
  ...
  static new(args: Arguments): MarkovProcessGraph {
    ...
    const personalAttributionsMap = new PersonalAttributionsMap(
      personalAttributions,
      epochStarts
    );
  ...
  *inNeighbors(nodeAddress: NodeAddressT): Iterator<MarkovEdge> {
  ...
  toJSON(): MarkovProcessGraphJSON {
    ...
    return {
      ...
      indexedMints,
      personalAttributions: this._personalAttributionsMap.toPersonalAttributions(),
    };
  }
...
function* virtualizedMarkovEdgeAddresses(
  ...
  for (const epochStart of epochStarts) {
    for (const {id} of participants) {
      ...
      for (const toParticipantId of personalAttributionsMap.recipientsForEpochAndParticipant(
        epochStart,
        id
      )) {
...
function virtualizedMarkovEdge(
  ...
  if (MarkovEdgeAddress.hasPrefix(address, payoutGadget.prefix)) {
    ...
    const sumOfPersonalAttributions =
      personalAttributionsMap.getSumProportionValue(
  ...
  if (MarkovEdgeAddress.hasPrefix(address, personalAttributionGadget.prefix)) {
    ...
    const proportionValue =
      personalAttributionsMap.getProportionValue(
